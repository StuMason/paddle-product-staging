	...
	return function PaddleConstruct(_window, libraryVersion) {
		...
		var _env = {
			...
			defaults: {
				...
				dev: {
					checkoutBase: 'https://checkout.development.paddle-internal.com/checkout/product/',
					internalCheckoutBase: 'https://checkout.development.paddle-internal.com/checkout/tool/',
					pricesApi: 'https://checkout.development.paddle-internal.com/api/1.0/prices',
					dataApi: 'https://checkout.development.paddle-internal.com/api/1.0/data',
					orderApi: 'https://checkout.development.paddle-internal.com/api/1.0/order',
					audienceApi: 'https://checkout.development.paddle-internal.com/api/1.0/audience/{vendor_id}/add',
					userHistoryApi: 'https://checkout.development.paddle-internal.com/api/2.0/user/history',
					analyticsKey: '8b03159305e2c0f49bf7481c073a4819',
					affiliateAnalyticsKey: '8be7b8d69526697e7ae22aff30d34603',
					consentMessageTranslationAPI: 'https://checkout.development.paddle-internal.com/ajax/consent-message'
				},
				staging: {
					checkoutBase: 'https://checkout.staging.paddle-internal.com/checkout/product/',
					internalCheckoutBase: 'https://checkout.staging.paddle-internal.com/checkout/tool/',
					pricesApi: 'https://checkout.staging.paddle-internal.com/api/1.0/prices',
					dataApi: 'https://checkout.staging.paddle-internal.com/api/1.0/data',
					orderApi: 'https://checkout.staging.paddle-internal.com/api/1.0/order',
					audienceApi: 'https://checkout.staging.paddle-internal.com/api/1.0/audience/{vendor_id}/add',
					userHistoryApi: 'https://checkout.staging.paddle-internal.com/api/2.0/user/history',
					analyticsKey: '8b03159305e2c0f49bf7481c073a4819',
					affiliateAnalyticsKey: '8be7b8d69526697e7ae22aff30d34603',
					consentMessageTranslationAPI: 'https://checkout.staging.paddle-internal.com/ajax/consent-message'
				},
				production: {
		...
		this.Order = {
			...
			DetailsPopup: function(checkoutHash, processingMessage) {
				var popupId = '_'+Math.ceil(Math.random()*10000000);
				...
				var popupHtml = '<div class="paddle-popup paddle-animated paddle-bounceIn">';
					...
					popupHtml += '<div class="paddle-popup-inner paddle-no-padding" style="background-color: #FFFFFF !important;">';
						popupHtml += '<div class="paddle-popup-order-loading paddle-popup-order-loading_'+popupId+'">';
							...
							popupHtml += '<div class="paddle-popup-order-loading-text paddle-order-loading-text_'+popupId+'">'+processingMessage+'</div>';
						popupHtml += '</div>';
						
						popupHtml += '<div class="paddle-popup-order-error paddle-popup-order-error_'+popupId+' paddle-fadeInDown paddle-hidden">';
							popupHtml += 'Your receipt and purchase information have been sent to the email address used during purchase.';
						popupHtml += '</div>';
						
						popupHtml += '<div class="paddle-popup-order paddle-popup-order_'+popupId+' paddle-fadeInDown paddle-hidden">';
				...
				_util.ready(function() {
					...
					orderPopup.setAttribute('class', 'paddle-reset paddle-popup-container paddle-popup-instance_'+popupId+' paddle-animated paddle-fadeIn paddle-hidden');
					orderPopup.innerHTML = popupHtml;
					body.appendChild(orderPopup);
					
					var close = document.getElementsByClassName('paddle-popup-instance_'+popupId)[0].getElementsByClassName('paddle-popup-close-image')[0];
					close.onclick = function(e) {
						e.preventDefault();
						_this.Popup.hide(popupId, 'order');
					};
					
					_this.Popup.show(popupId, 'Order', 'order');
					...
					_this.Order.details(checkoutHash, function(data) {
						if(data) {
							if(data.state == 'processed') {								
								var popupContent = '<div class="paddle-popup-order-details">';
									...
									popupContent += '<div class="paddle-popup-order-summary">';
										...
										popupContent += '<div class="paddle-popup-order-receipt">';
											popupContent += '<a href="'+data.order.receipt_url+'" target="_blank" class="paddle-popup-order-receipt-button paddle-popup-order-receipt-button_'+popupId+'">View Receipt</a>';
										popupContent += '</div>';
													...
													popupContent += '<div class="paddle-popup-locker-item">';
														...
														if(typeof locker.download != 'undefined' && locker.download != '') {
															popupContent += '<div class="paddle-popup-locker-row-button">';
																popupContent += '<a href="'+locker.download+'" target="_blank" class="paddle-popup-locker-row-button-link paddle-popup-order-download-button_'+popupId+'">Download</a>';
															popupContent += '</div>';
														}
									...
									popupContent += '<div class="paddle-popup-order-problem">';
										popupContent += 'Something wrong? <a href="mailto:help+pjs_'+data.order.order_id+'_'+checkoutHash+'_order@paddle.com" class="paddle-popup-order-problem-link paddle-popup-order-problem-link_'+popupId+'">Contact Support</a>';
								...
								var orderPopupContent = document.getElementsByClassName('paddle-popup-order_'+popupId)[0];
								orderPopupContent.innerHTML = popupContent;
								
								_util.hide(document.getElementsByClassName('paddle-popup-order-loading_'+popupId)[0]);
								
								// Add analytics events to buttons/links within the popup.
								if(typeof document.getElementsByClassName('paddle-popup-order-problem-link_'+popupId)[0] != 'undefined') {
									document.getElementsByClassName('paddle-popup-order-problem-link_'+popupId)[0].onclick = function(e) {
										_this.Analytics.track('Paddle.Order.Details.Contact', {}, {});
									};
								}
								
								if(typeof document.getElementsByClassName('paddle-popup-order-download-button_'+popupId)[0] != 'undefined') {
									document.getElementsByClassName('paddle-popup-order-download-button_'+popupId)[0].onclick = function(e) {
								...
								if(typeof document.getElementsByClassName('paddle-popup-order-receipt-button_'+popupId)[0] != 'undefined') {
									document.getElementsByClassName('paddle-popup-order-receipt-button_'+popupId)[0].onclick = function(e) {
								...
								_util.hide(document.getElementsByClassName('paddle-popup-order-loading_'+popupId)[0]);
								_util.show(document.getElementsByClassName('paddle-popup-order-error_'+popupId)[0]);
							}
						} else {
							_util.hide(document.getElementsByClassName('paddle-popup-order-loading_'+popupId)[0]);
							_util.show(document.getElementsByClassName('paddle-popup-order-error_'+popupId)[0]);
		...
		this.Download = {
				...
				_util.ready(function() {					
					...
					_util.each('paddle_download', function(buttonElement) {
						...
						if(!_buttonHasInit) {
							...
							var vendorName = buttonElement.getAttribute('data-vendor-name') || '';

							var popupId = _this.Audience.Popup({
								...
								callback: function(data) {
									if(data.success) {
										_this.Analytics.track('Paddle.Download.Prompt.Success', {}, {
											'ProductID': downloadProductId || false,
											'DownloadUrl': downloadUrl
										});
										
										_this.Download.Start(downloadUrl, downloadProductId);
									}
								}
							});
							...
							buttonElement.onclick = function(e) {
								...
								if(downloadProductId) {
									...
									if(productDownloadUrl) {
										if(prompt) {
											_this.Download.StartWithPrompt(productDownloadUrl, downloadProductId, popupId);
										} else {
									...
									if(prompt) {
										_this.Download.StartWithPrompt(downloadUrl, false, popupId);
									} else {
			...
			StartWithPrompt: function(url, productId, popupId) {
				...
				_this.Popup.show(popupId, 'Download', 'download');
			},
		...
		this.Popup = {
			show: function(popupId, method, type) {
				type = type || 'popup';
				method = method || 'Manual'; // manual / timed / scrollDepth / exitIntent / etc...
				...
				if(method == 'Manual' || method == 'Download' || method == 'Order' || _this.Audience.AllowPopup()) {	
					...
					var popup = document.getElementsByClassName('paddle-popup-instance_'+popupId)[0] || false;
					if(popup) {
			...
			hide: function(popupId, type) {
				var type = typeof type != 'undefined' ? type : 'popup';
				var popup = document.getElementsByClassName('paddle-popup-instance_'+popupId)[0] || false;
				var popupBox = document.getElementsByClassName('paddle-popup-instance_'+popupId)[0].getElementsByClassName('paddle-popup')[0] || false;
		...
		this.ConsentMessage = {
			...
			getBaseMessageWithVendor: function(vendorName) {
				return this.replaceVendorName(this.base, vendorName);
			},

			getTranslatedMessageWithVendor: function(vendorName) {
				return this.replaceVendorName(this.translated, vendorName);
			},

			replaceVendorName: function(message, vendorName) {
				return message.replace(/<var(.*?)var>/, vendorName)
			},
			...
			fetch: function() {
				...
				_util.ajaxRequest(
					_this.Environment.defaults().consentMessageTranslationAPI,
					'GET',
					{},
					function processConsentMessageTranslation(response) {
						try {
							var parsedResponse = JSON.parse(response);
							var parsedResponseData = parsedResponse.data || {};

							_me.translated = parsedResponseData.message || '';
							_me.base = parsedResponseData.base || '';

							_me.isFetching = false;

							var audiencePopups = document.querySelectorAll('.paddle-popup');
							...
							if (audiencePopups.length > 0) {
								for (var i = audiencePopups.length - 1; i >= 0; i--) {
									var $popup = audiencePopups[i];
									$popup.removeAttribute('aria-busy');

									var $marketingConsent = $popup.querySelector('.paddle-popup-checkbox');

									if ($marketingConsent) {
										var $marketingConsentLabel = $marketingConsent.querySelector('.paddle-popup-checkbox-label');
										var labelVendorName = $marketingConsentLabel.getAttribute('data-vendor-name');

										$marketingConsent.removeAttribute('style');
										$marketingConsentLabel.innerHTML = _me.getTranslatedMessageWithVendor(labelVendorName);
									}
								}
							}
						} catch (error) {
		...
		this.Audience = {
			...
			GeneratePopupHTML: function(popupId, attributes) {
				var popupHtml = '<div class="paddle-popup paddle-animated paddle-'+attributes.view.animations.show+'" aria-busy="true">';
					...
					popupHtml += '<div class="paddle-popup-inner" style="background-color: '+attributes.view.styles.popup.backgroundColor+'; '+popupBackground+'">';
						...
						popupHtml += '<form class="paddle-popup-form">';
							...
							if (attributes.isGdprEnabled) {
								popupHtml += '<label class="paddle-popup-checkbox" for="newsletter-consent-input-'+ popupId +'" style="display: none !important;">';
									popupHtml += '<input class="paddle-popup-checkbox-input" id="newsletter-consent-input-'+ popupId +'" type="checkbox">';
									popupHtml += '<span id="paddle-popup-marketing-consent-message" class="paddle-popup-checkbox-label" data-vendor-name="' + attributes.vendorName + '">'+ _this.ConsentMessage.translated +'</span>';
								popupHtml += '</label>';
							}

							popupHtml += '<input type="submit" class="paddle-popup-cta" value="'+attributes.strings.cta+'" style="color: '+attributes.view.styles.cta.textColor+'; background-color: '+attributes.view.styles.cta.backgroundColor+';" />';
						popupHtml += '</form>';
			...
			Popup: function(inputAttributes) {
				...
				this.vendorName = inputAttributes.vendorName || '';
				var isGdprEnabled = inputAttributes.vendorName ? true : false;
				var popupId = '_'+Math.ceil(Math.random()*10000000);
				...
				_util.ready(function() {
					...
					audiencePopup.setAttribute('class', 'paddle-reset paddle-popup-container paddle-popup-instance_'+popupId+' paddle-animated paddle-fadeIn paddle-hidden');
					audiencePopup.innerHTML = _this.Audience.GeneratePopupHTML(popupId, attributes);
					body.appendChild(audiencePopup);

					// Fetches the translated consent messages only if they haven't been fetched yet.
					if (!_this.ConsentMessage.isFetching && !_this.ConsentMessage.translated) {
						_this.ConsentMessage.fetch();
					}

					var popupDomElement = document.getElementsByClassName('paddle-popup-instance_'+popupId)[0];
					var marketingConsentCheckboxElement = popupDomElement.getElementsByClassName('paddle-popup-checkbox-input')[0];
					var marketingConsentLabelElement = popupDomElement.getElementsByClassName('paddle-popup-checkbox-label')[0];
					...
					var close = popupDomElement.getElementsByClassName('paddle-popup-close-image')[0];
					close.onclick = function(e) {
						e.preventDefault();
						_this.Popup.hide(popupId, 'audience');
					};
					
					var popupForm = popupDomElement.getElementsByClassName('paddle-popup-form')[0];
					popupForm.onsubmit = function(event) {
						event.preventDefault();

						if (!attributes.isGdprEnabled || marketingConsentCheckboxElement.checked) {
							_this.Audience.popupSubmit(popupId, attributes);
						} else {
					...
					if(attributes.triggers.exitIntent) {
						...
						var checkPopupTrigger = function(e) {
							if(e.clientY <= mouseLeaveOffset) {
								_this.Popup.show(popupId, 'ExitIntent', 'audience');
							}
						}
								...
								window.checkScrollDepth = setTimeout(function() {
									...
									if(scrollDepth >= attributes.triggers.scrollDepth) {
										if(!_status.AudienceHasPopped) {
											...
											_this.Popup.show(popupId, 'ScrollDepth', 'audience');
										}
									}
								}, 300);
							};
					...
					if(attributes.triggers.timed) {
						...
						setTimeout(function() {
							_this.Popup.show(popupId, 'Timed', 'audience');
			...
			emailFromPopup: function(popupId) {
				var email = document.getElementsByClassName('paddle-popup-instance_'+popupId)[0].getElementsByClassName('paddle-popup-email')[0] || false;
				if(email) {
					return email.value;
				} else {
					return false;
			...
			popupSubmit: function(popupId, popupAttributes) {
				_this.Debug("Audience popup submitted.");
				
				var email = _this.Audience.emailFromPopup(popupId);
				var callback = popupAttributes.callback || function() {};
				var successMessage = '';

				try {
					successMessage = popupAttributes.strings.successMessage;
				} catch (error) {
					_this.Debug('The success message attribute is not valid');
				}

				var newAudienceMemberAttributes = {
					marketing_consent: 0,
					email: email,
					consent_collected_by: 'vendor'
				};

				if (popupAttributes.isGdprEnabled && popupAttributes.vendorName) {
					newAudienceMemberAttributes.marketing_consent = 1;
					newAudienceMemberAttributes.consent_collected_by = 'paddle';
					newAudienceMemberAttributes.marketing_consent_message_base = _this.ConsentMessage.getBaseMessageWithVendor(popupAttributes.vendorName);
					newAudienceMemberAttributes.marketing_consent_message_localised = _this.ConsentMessage.getTranslatedMessageWithVendor(popupAttributes.vendorName);
				}
		...
		this.Checkout = {
			...
			open: function(checkoutAttributes, invocationMethod) {				
				_util.closeCheckout({}, false, false);
				...
				if(typeof checkoutAttributes == 'object') {
					...
					if(typeof checkoutAttributes.checkoutVersion != 'undefined') {
						checkoutAttributes.checkout_version = checkoutAttributes.checkoutVersion;
						delete checkoutAttributes.checkoutVersion;
					}
					...
					checkoutAttributes.checkout_layout_forced = _options.checkoutVariant.isForced ? 'true' : 'false';
					...
					if(checkoutMethod == 'overlay' && typeof checkoutAttributes.upsell != 'undefined' && checkoutAttributes.upsell != '') {
						...
						_util.jsonp(_this.Environment.defaults().dataApi+'?product_id='+checkoutAttributes.upsell, function(data) {
							...
							var success = (typeof checkoutAttributes.success != 'undefined') ? checkoutAttributes.success : '';
							
							_this.Upsell.create(checkoutAttributes.upsell, imageUrl, title, description, ctaText, orginalCheckout, passthrough, upsellCoupon, success);
						});
					} else {
						var hasUpsell = false;
					}
		...
		this.Upsell = {
			...
			create: function(productId, imageUrl, title, description, ctaText, orginalCheckoutFunction, passthrough, upsellCoupon, success) {
				...
				window['paddleSuccessCallback'] = window[_activeCheckout.successCallback] || _activeCheckout.successCallback || null;
				window['paddleCloseCallback'] = window[_activeCheckout.closeCallback] || _activeCheckout.closeCallback || null;

				// Checkbox vs Button Upsell
				if(_options.upsellCheckbox) {
					window.upsellType = 'Checkbox';
					var upsellAction = '<div class="paddle_upsell-cta-checkbox-container"><div class="paddle_upsell-cta paddle_upsell-cta-checkbox"><input type="checkbox" class="paddle_upsell-checkbox" id="paddle_upsell-checkbox" onchange="javascript:document.getElementsByClassName(\'paddle_upsell_button\')[0].click();" /> <label for="paddle_upsell-checkbox">'+ctaText+'</label><a href="#!" class="paddle_button paddle_upsell_button" style="visibility:none;" data-checkout-version="'+(_activeCheckout.checkout_version || "")+'" data-theme="none" data-product="'+productId+'" data-upsell-button="true" data-referrer="Upsell" data-passthrough="'+passthrough+'" data-success="'+success+'" data-close-callback="paddleCloseCallback" data-success-callback="paddleSuccessCallback" data-coupon="'+upsellCoupon+'" data-marketing-consent="'+(_activeCheckout.marketing_consent || "")+'" data-email="'+(_activeCheckout.guest_email || "")+'" data-country="'+(_activeCheckout.guest_country || "")+'" data-postcode="'+(_activeCheckout.guest_postcode || "")+'"></a></div></div>';
				} else {
					window.upsellType = 'Button';
					var upsellAction = '<div class="paddle_upsell-cta"><a href="#" class="paddle_upsell_button paddle_button" data-product="'+productId+'" data-checkout-version="'+(_activeCheckout.checkout_version || "")+'" data-upsell-button="true" data-referrer="Upsell" data-passthrough="'+passthrough+'" data-success="'+success+'" data-close-callback="paddleCloseCallback" data-success-callback="paddleSuccessCallback" data-coupon="'+upsellCoupon+'" data-marketing-consent="'+(_activeCheckout.marketing_consent || "")+'" data-email="'+(_activeCheckout.guest_email || "")+'" data-country="'+(_activeCheckout.guest_country || "")+'" data-postcode="'+(_activeCheckout.guest_postcode || "")+'">'+ctaText+'</a></div>';
				}
		...
		this.Button = {
			...
			getButtonAttributes: function(buttonElement) {
				...
				// Guest Email
				buttonAttributes = _this.Button.attribute(buttonAttributes, 'checkout_version', buttonElement, 'data-checkout-version', '');

				// Marketing Consent
				if (buttonElement.hasAttribute('data-marketing-consent')) {
					var isMarketingConsentEnabled =
						buttonElement.getAttribute('data-marketing-consent') == 'true' ||
						buttonElement.getAttribute('data-marketing-consent') == '1';
		...
		var _util = {
			...
			// Hide the checkout iframe with CSS on initial load in Safari
						...
						if(message.data.action == 'event') {
							...
							if(message && message.data && message.data.event_name) {
								...
								if(message.data.event_name == 'Checkout.Loaded') {
									// If the checkout frame is hidden, show it when we recieve the loaded event.
			...
			showPoweredBy: function() {
				_this.Debug('"showPoweredBy" method has been deprecated.');
			},
			
			// Hides 'powered by paddle' badge.
			hidePoweredBy: function() {
				_this.Debug('"hidePoweredBy" method has been deprecated.');
			},
			...
			buildCheckoutUrl: function(productId, checkoutQuery, type) {
				...
				if(checkoutQuery.internal == 'true') {
					...
					if(_util.isAffiliate() && (typeof checkoutQuery.override == 'undefined' || checkoutQuery.override.length == 0)) {
						...
						var checkoutFullUrl = 'https://8sktjl23oi.execute-api.us-east-1.amazonaws.com/Prod/checkout/'+_util.affiliateToken()+'/?type=product&product_id='+productId+'&'+_util.queryString(checkoutQuery);
			...
			renderCheckoutFrame: function(productId, checkoutQuery, inline) {
				...
				if(typeof checkoutQuery.frameStyle != 'undefined' && checkoutQuery.frameStyle != '') {
					...
					if(!_util.isMobile()) {
						window.PaddleFrame.setAttribute('style', 'z-index: 99999; display: '+_util.initialCheckoutVisibility()+'; background-color: transparent; border: 0px none transparent; overflow-x: hidden; overflow-y: auto; visibility: visible; margin: 0px; padding: 0px; -webkit-tap-highlight-color: transparent; position: fixed; left: 0px; top: 0px; width: 100%; height: 100%;');
					} else {
			...
			completeCheckout: function(callback_data) {
				...
				if(typeof window[_activeCheckout.successCallback] == 'function') {
					_util.closeCheckout({}, false, true);
...
(function() {
	var Paddle;
	var PaddleVersion = "1.9.34"; // Increment this upon new Paddle.js version.
